...
export function Header({ config }: HeaderProps) {
  const logoStyle: CSSProperties = isPlaceholderUrl(config.client.logo)
    ? { background: `linear-gradient(135deg, ${config.client.primaryColor}, #0f241a)` }
      ...
      <div className="mx-auto flex h-full max-w-6xl items-center justify-between px-4 sm:px-6">
        <Link href="/" className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-[#1C1917] sm:text-base">{config.client.brandName}</p>
