import type { PricingTier, PricingUnit } from "@/types";
...
export function PricingLegend({ tiers, pricingUnit }: PricingLegendProps) {
  return (
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {tiers.map((tier) => (
          <div key={tier.id} className="flex items-center gap-2 text-sm text-[#78716C]">
            ...
            <span className="font-medium text-[#1C1917]">{formatRate(tier.ratePerHour, pricingUnit)}</span>
          </div>
        ))}
      </div>
    </div>
