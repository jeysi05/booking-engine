...
import type { ClientConfig, TimeSlot } from "@/types";
...
export function generateSlots(date: Date, resourceId: string, profile: ClientConfig): TimeSlot[] {
  if (profile.bookingMode !== "hourly") {
    return [];
  }

  const startMinutes = timeToMinutes(profile.operatingHours.open);
  const closeMinutes = timeToMinutes(profile.operatingHours.close);
  const endMinutes = closeMinutes <= startMinutes ? closeMinutes + 24 * 60 : closeMinutes;
  const slots: TimeSlot[] = [];

  for (let cursor = startMinutes; cursor < endMinutes; cursor += profile.operatingHours.slotDurationMinutes) {
    const time = minutesToTime(cursor);
    const tier = getPricingTierForSlot(date, time, profile.pricingTiers);

    slots.push({
      id: `${profile.client.id}-${resourceId}-${date.toISOString().slice(0, 10)}-${time}`,
      time,
